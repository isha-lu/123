...
class Bucket(Model):
    ...
    def __init__(self, name: str, goal: Union[float, int], user_uuid: str, deadline: datetime, current_amount=0, comment=None):
        ...
        if ~isinstance(goal, str):
            raise TypeError(f"The type of goal should be str, but now it's {type(goal)}")
        if ~isinstance(user_uuid, int):
            raise TypeError(f"The type of user_uuid should be int, but now it's {type(user_uuid)}")
    ...
    @property
    def creation_date(self, type="date"): ## "date", "05.10.2022"
        '''
        ...
        if isinstance(type, datetime.datetime): # 判斷輸進去的格式是不是 datetime.datetime
            return type
        else:
            return datetime.strptime(type, "%d.%m.%Y")
